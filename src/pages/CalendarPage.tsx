...
const CalendarPage = () => {
  ...
  const { employees, calendarTasks, addCalendarTask, deleteCalendarTask, convertTaskToCard } = useApp();
  ...
  const getTasksForDay = (day: number) => calendarTasks.filter(t => t.date === getDateStr(day));

  const handleAdd = (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedDate || !form.clientName.trim()) return;
    addCalendarTask({ ...form, date: selectedDate, imageUrl: form.imageUrl || undefined });
    setForm({ clientName: '', contentType: '', description: '', time: '09:00', imageUrl: '', status: 'pendente', employeeId: '' });
    setShowAddDialog(false);
  };

  const dayTasks = selectedDate ? calendarTasks.filter(t => t.date === selectedDate) : [];
  ...
  return (
    <div className="min-h-screen bg-background p-6">
      <header className="flex items-center gap-4 mb-8">
        ...
        <h1 className="text-2xl font-bold gradient-text">Calendário</h1>
      </header>
      ...
      <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
        <DialogContent className="bg-card border-border">
          ...
          <form onSubmit={handleAdd} className="space-y-3">
            ...
            <Button type="submit" className="w-full bg-primary text-primary-foreground">Criar Tarefa</Button>
          </form>
        </DialogContent>
      </Dialog>
    </div>
