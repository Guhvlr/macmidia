          ...
          {localImages.map((img, i) => {
             ...
             return (
              <div 
                ...
                className={`group relative rounded-xl overflow-hidden bg-black/40 border border-white/10 aspect-video cursor-grab active:cursor-grabbing flex items-center justify-center ${draggedIdx === i ? 'opacity-50' : ''}`}
              >
                {isImg ? (
                  <>
                    <img src={img} alt="" className="absolute inset-0 w-full h-full object-cover blur-xl opacity-50 scale-110" />
                    <img src={img} className="relative z-10 max-w-full max-h-full object-contain drop-shadow-lg" alt={`Anexo ${i + 1}`} />
                  </>
                ) : (
