...
export function CanvasEditor({ pageIndex, onClose }: { pageIndex: number, onClose: () => void }) {
  ...
  const [localSlotSettings, setLocalSlotSettings] = useState<Record<string, any>>(() => ({ ...context.slotSettings }));
  const [localPriceBadge] = useState(() => ({ ...context.priceBadge }));
  const [localDescConfig] = useState(() => ({ ...context.descConfig }));
  const [localImageConfig] = useState(() => ({ ...context.imageConfig }));

  const getSlotSettings = useCallback((slotId: string) => {
    const global = context.getSlotSettings(slotId);
    const local = localSlotSettings[slotId] || {};
    return {
      priceBadge: { ...global.priceBadge, ...(local.priceBadge || {}) },
      descConfig: { ...global.descConfig, ...(local.descConfig || {}) },
      imageConfig: { ...global.imageConfig, ...(local.imageConfig || {}) },
    };
  }, [localSlotSettings, context]);

  const updateSlotSettings = useCallback((slotId: string, newSettings: any) => {
  ...
  const saveAndClose = async () => {
    ...
    Object.entries(localSlotSettings).forEach(([slotId, settings]) => {
      context.updateSlotSettings(slotId, settings);
    });
  ...
  const handleDragEnd = (e: any, slotId: string, gIdx: number, type: string) => {
    const node = e.target;
    const slotBase = context.slots.find(s => s.id === slotId);
    if (!slotBase) return;
    
    const cfg = getSlotSettings(slotId);
    ...
    if (Object.keys(up).length > 0) updateSlotSettings(slotId, up);
  };
  ...
  const handleTransformEnd = (e: any, gIdx: number, type: string) => {
    ...
    const slotBase = context.slots[gIdx];
    const cfg = getSlotSettings(slotBase.id);
    const sf = slotBase.width / 500;
    
    if (type === 'image') {
      const newScale = nw / (slotBase.width * 0.8);
      updateSlotSettings(slotBase.id, { imageConfig: { ...cfg.imageConfig, scale: newScale } });
    } else if (type === 'name') {
      const newFontSize = nh / (sf * 1.5);
      updateSlotSettings(slotBase.id, { descConfig: { ...cfg.descConfig, fontSize: newFontSize } });
    } else if (type === 'badge') {
      updateSlotSettings(slotBase.id, { priceBadge: { ...cfg.priceBadge, badgeWidth: nw/sf, badgeHeight: nh/sf } });
    }
  };
  ...
  const saveInlineEdit = () => {
    ...
    const slotBase = context.slots[slotIdx];
    ...
    if (editingElem.type === 'name') {
      updateSlotSettings(slotBase.id, { descConfig: { ...cfg.descConfig, fontSize: editFontSize } });
    } else if (editingElem.type === 'price') {
      updateSlotSettings(slotBase.id, { priceBadge: { ...cfg.priceBadge, valueFontSize: editFontSize } });
    } else if (editingElem.type === 'suffix') {
      updateSlotSettings(slotBase.id, { priceBadge: { ...cfg.priceBadge, suffixText: editValue, suffixFontSize: editFontSize } });
    }
          ...
          <div style={{ transform: `scale(${zoom})`, transformOrigin: 'center center' }}>
            ...
            >
                <Layer>
                    ...
                    {context.slots
                        .map((slotBase, gIdx) => ({ slotBase, gIdx }))
                        .filter(({ slotBase }) => (slotBase.pageIndex || 0) === pageIndex)
                        .map(({ slotBase, gIdx }) => {
                            ...
                            >
                                {product.images?.[0] && (
                                    <URLImage 
                                        ...
                                        onDragEnd={e => handleDragEnd(e, slotBase.id, gIdx, 'image')}
                                ...
                                {cfg.priceBadge.badgeImageUrl ? (
                                    <URLImage 
                                        ...
                                        onDragStart={pushHistory}
                                        onDragEnd={e => handleDragEnd(e, slotBase.id, gIdx, 'badge')}
                                ...
                                <KonvaText
                                    ...
                                    onDragStart={pushHistory}
                                    onDragEnd={e => handleDragEnd(e, slotBase.id, gIdx, 'currency')}
                                ...
                                <KonvaText
                                    ...
                                    onDragStart={pushHistory}
                                    onDragEnd={e => handleDragEnd(e, slotBase.id, gIdx, 'value')}
                                ...
                                {cfg.priceBadge.showSuffix && (
                                   <KonvaText
                                      ...
                                      onDragStart={pushHistory}
                                      onDragEnd={e => handleDragEnd(e, slotBase.id, gIdx, 'suffix')}
                                ...
                                <KonvaText
                                    ...
                                    onDragStart={pushHistory}
                                    onDragEnd={e => handleDragEnd(e, slotBase.id, gIdx, 'name')}
