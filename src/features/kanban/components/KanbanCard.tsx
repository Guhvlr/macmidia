...
const KanbanCardInner = ({ card, employees, updateKanbanCard, triggerAICorrection, dragAttributes, dragListeners }: KanbanCardProps) => {
  ...
  // Stuck Card Detection (>24h without activity and not in final column)
  const isCompletedCol = isFinalColumn(card.column);
  let latestTime = 0;
  ...
  const hoursInactive = latestTime > 0 ? Math.floor((Date.now() - latestTime) / (1000 * 60 * 60)) : 36;
  const isStuck = !isCompletedCol && hoursInactive >= 24;
      ...
      <div
        ...
        className={`kanban-card bg-[#1C1C1E] border rounded-xl p-3 space-y-3 cursor-pointer group hover:bg-[#252528] hover:border-white/10 active:scale-[0.98] transition-all duration-200 shadow-md relative overflow-hidden flex flex-col performance-virtual ${
          isStuck 
            ...
            : 'border-white/5'
        } ${isDragOver ? 'ring-2 ring-primary ring-offset-2 ring-offset-black scale-[1.02] bg-[#252528]' : ''}`}
      >
        {isStuck && (
          <div className="absolute top-2 right-2 z-20">
            <span className="px-2 py-0.5 text-[9px] font-black uppercase bg-red-600 text-white rounded-md flex items-center gap-1 shadow-lg border border-red-400 animate-pulse">
              <AlertTriangle className="w-3 h-3 text-white" /> Parado {formatInactiveTime(hoursInactive)}
            </span>
          </div>
        )}
        ...
        <div className="space-y-2 flex-1">
          ...
          {/* Trello-style Date Badge on Card Preview */}
          {(card.dueDate) && (
            <div className={`flex items-center gap-1.5 mt-2 px-2 py-1 rounded-md w-fit ${
        ...
        <ContextMenuContent className="bg-[#1a1a1c] border-white/10 rounded-xl shadow-2xl min-w-[180px] z-[99999]">
          <ContextMenuItem 
            onClick={handleOpenDetail}
            className="text-[12px] font-semibold text-white/80 hover:bg-white/10 cursor-pointer rounded-lg px-3 py-2"
          >
            Abrir cartão
          </ContextMenuItem>
