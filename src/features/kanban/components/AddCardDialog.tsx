...
const AddCardDialog = ({ employeeId: initialEmployeeId, fixedColumnKey, columnKey, trigger, open, onOpenChange, showEmployeeSelect }: Props) => {
  const { addKanbanCard, uploadKanbanAsset, employees, calendarClients, loggedUserRole, loggedUserClientLink } = useApp();
  const [internalOpen, setInternalOpen] = useState(false);
  const [clientName, setClientName] = useState('');
  const [selectedCalendarClientId, setSelectedCalendarClientId] = useState('');

  // Filtrar clientes para visitantes (GUEST) - mostrar apenas os vinculados
  const allowedClients = useMemo(() => {
    if (loggedUserRole === 'GUEST' && loggedUserClientLink) {
      const allowedIds = loggedUserClientLink.split(',').map(id => id.trim());
  ...
  // Auto-selecionar cliente se visitante tem apenas 1 vinculado
  useEffect(() => {
    if (dialogOpen && allowedClients.length === 1 && !selectedCalendarClientId) {
      setSelectedCalendarClientId(allowedClients[0].id);
    }
  }, [dialogOpen, allowedClients, selectedCalendarClientId]);
...
Retorne JSON: {"clientName": "...", "description": "..."}`;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      toast.error(`Erro ao criar card: ${err?.message || 'Erro desconhecido'}`);
    } finally {
