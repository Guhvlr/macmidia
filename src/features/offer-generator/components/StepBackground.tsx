...
export const StepBackground = () => {
  ...
  const [layoutToDelete, setLayoutToDelete] = React.useState<string | null>(null);
  ...
  const confirmDelete = async () => {
    ...
    try {
      const { error } = await (supabase as any).from('offer_layouts').delete().eq('id', layoutToDelete);
      if (error) throw error;
      setLayouts(prev => prev.filter(ly => ly.id !== layoutToDelete));
      toast.success('Layout excluído!');
    } catch (e: any) { toast.error(e.message); }
    finally { setLayoutToDelete(null); }
  };
  ...
  return (
    <div className="h-full flex bg-zinc-950">
      ...
      <AlertDialog open={!!layoutToDelete} onOpenChange={(o) => !o && setLayoutToDelete(null)}>
        <AlertDialogContent className="bg-zinc-950 border-zinc-800 rounded-[24px]">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-zinc-400 font-medium">
              Deseja excluir este layout permanentemente? Esta ação não pode ser desfeita.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel className="bg-zinc-900 border-zinc-800 text-zinc-300 hover:bg-zinc-800 hover:text-zinc-100 rounded-xl font-medium">Cancelar</AlertDialogCancel>
            <AlertDialogAction onClick={confirmDelete} className="bg-red-600 hover:bg-red-500 text-white rounded-xl font-semibold shadow-md shadow-red-900/20">Excluir Permanentemente</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
