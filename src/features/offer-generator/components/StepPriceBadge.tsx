...
export const StepPriceBadge = () => {
  ...
  const upPreset = useCallback((preset: any) => {
    pushHistory();
    const targets = selectedSlotIds.length > 0 ? selectedSlotIds : (selectedSlotId ? [selectedSlotId] : []);
    if (targets.length === 0) return;

    targets.forEach(id => {
      const targetSlot = slots.find(s => s.id === id);
      const targetSf = (targetSlot?.width || 500) / 500;
      const sourceSf = (preset.sourceWidth || 500) / 500;
      const ratio = sourceSf / targetSf;

      const compensatedDesc = preset.descConfig ? {
        ...preset.descConfig,
        fontSize: preset.descConfig.fontSize * ratio,
      } : undefined;

      const compensatedBadge = preset.priceBadge ? {
        ...preset.priceBadge,
        badgeWidth: preset.priceBadge.badgeWidth * ratio,
        badgeHeight: preset.priceBadge.badgeHeight * ratio,
        valueFontSize: preset.priceBadge.valueFontSize * ratio,
        currencyFontSize: preset.priceBadge.currencyFontSize * ratio,
        suffixFontSize: preset.priceBadge.suffixFontSize * ratio,
      } : undefined;

      const updates: any = {};
      if (compensatedDesc) updates.descConfig = compensatedDesc;
      if (compensatedBadge) updates.priceBadge = compensatedBadge;

      updateSlotSettings(id, updates);
    });
    toast.success('Modelo aplicado (tamanho mantido)!');
  }, [selectedSlotId, selectedSlotIds, slots, updateSlotSettings, pushHistory]);
    ...
    const handleKeys = (e: KeyboardEvent) => {
      if ((e.ctrlKey || e.metaKey)) {
        ...
        if (e.key.toLowerCase() === 'c' && selectedSlotId !== null) {
          e.preventDefault();
          const sourceSlot = slots.find(s => s.id === selectedSlotId);
          setClipboard({
            settings: getSlotSettings(selectedSlotId),
            sourceWidth: sourceSlot?.width || 500
          });
        ...
        if (e.key.toLowerCase() === 'v' && clipboard && selectedSlotIds.length > 0) {
          ...
          selectedSlotIds.forEach(id => {
            const targetSlot = slots.find(s => s.id === id);
            const targetSf = (targetSlot?.width || 500) / 500;
            const sourceSf = (clipboard.sourceWidth || 500) / 500;
            const ratio = sourceSf / targetSf;
            
            const src = clipboard.settings || clipboard; // support old clipboard format
            
            const compensatedDesc = src.descConfig ? {
              ...src.descConfig,
              fontSize: src.descConfig.fontSize * ratio,
            } : undefined;
            
            const compensatedBadge = src.priceBadge ? {
              ...src.priceBadge,
              badgeWidth: src.priceBadge.badgeWidth * ratio,
              badgeHeight: src.priceBadge.badgeHeight * ratio,
              valueFontSize: src.priceBadge.valueFontSize * ratio,
              currencyFontSize: src.priceBadge.currencyFontSize * ratio,
              suffixFontSize: src.priceBadge.suffixFontSize * ratio,
            } : undefined;
            
            const updates: any = {};
            if (src.imageConfig) updates.imageConfig = { ...src.imageConfig };
            if (compensatedDesc) updates.descConfig = compensatedDesc;
            if (compensatedBadge) updates.priceBadge = compensatedBadge;
            
            updateSlotSettings(id, updates);
          });
          toast.success('Estilo colado (tamanho mantido)!');
        }
      ...
      // ARROW KEYS MOVEMENT
      if ((e.key === 'ArrowUp' || e.key === 'ArrowDown' || e.key === 'ArrowLeft' || e.key === 'ArrowRight') && selectedSlotId !== null) {
        ...
        selectedSlotIds.forEach(id => {
          ...
          if (Object.keys(updates).length) updateSlotSettings(id, updates);
        });
      }
    };
  ...
  const getElems = (slot: any, cfg: any) => {
    ...
    return {
      ...
      name: { x: slot.x + slot.width * d.offsetX/100 - slot.width*0.4, y: slot.y + slot.height * d.offsetY/100 - (d.fontSize*sf)/2, w: slot.width*0.8, h: d.fontSize*sf*6 },
  ...
  const onMouseMove = (e: React.MouseEvent) => {
    ...
    if (dragging && selectedSlotId !== null) {
      ...
      let dx = c.x - dragOff.x - el.x;
      let dy = c.y - dragOff.y - el.y;
      if (e.shiftKey) {
  ...
  const hasName = selectedElems.includes('name');
  const hasBadge = selectedElems.includes('badge');
  const hasPrice = selectedElems.includes('currency') || selectedElems.includes('value');
  const hasSuffix = selectedElems.includes('suffix');
  const hasImage = selectedElems.includes('image');

  const selectedCategories = new Set<string>();
  if (hasName) selectedCategories.add('text');
  if (hasBadge) selectedCategories.add('badge');
  if (hasPrice) selectedCategories.add('price');
  if (hasSuffix) selectedCategories.add('suffix');
  if (hasImage) selectedCategories.add('image');

  const activeCategory = selectedCategories.size === 1 ? Array.from(selectedCategories)[0] : 'default';
    ...
    <div className="h-full flex overflow-hidden bg-zinc-950 text-zinc-100">
      ...
      <div className="w-[380px] bg-zinc-950 border-r border-zinc-800/60 p-8 overflow-y-auto custom-scrollbar flex flex-col gap-6 select-none">
        ...
        <div className="flex flex-col gap-3">
          {(activeCategory === 'default' || activeCategory === 'image') && (
            <Section label={`Modelos: ${selectedClientName || 'Geral'}`} icon={Zap} isOpen={true} onToggle={() => {}}>
               ...
               {filteredPresets?.map((p: any) => (
                  <div key={p.id} className="group flex items-center gap-2 p-2 bg-zinc-900/50 border border-zinc-800 rounded-xl hover:border-blue-500/40 transition-all">
                     <button onClick={() => upPreset(p)} className="flex-1 text-[12px] text-zinc-400 group-hover:text-zinc-200 text-left px-3 truncate font-medium">{p.name}</button>
          ...
          {activeCategory === 'default' && (
            <>
              {selectedCategories.size > 1 ? (
                 <div className="p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-xl text-yellow-400 text-[12px] text-center font-medium">
                    Múltiplos elementos de tipos diferentes selecionados. Selecione apenas elementos do mesmo tipo.
                 </div>
              ) : (
                 <div className="p-4 bg-blue-500/5 border border-blue-500/10 rounded-xl text-blue-400/80 text-[12px] text-center font-medium border-dashed">
                    Selecione um elemento no palco (como o fundo, o preço ou o título) para editar suas configurações.
                 </div>
              )}
            </>
          )}
          ...
          {activeCategory === 'badge' && (
            <Section label="Fundo do Preço" icon={Layers} isOpen={true} onToggle={() => {}}>
          ...
          {activeCategory === 'price' && (
            <Section label="Cores dos Textos (Preço)" icon={CreditCard} isOpen={true} onToggle={() => {}}>
              <div className="space-y-6">
                 <div className="space-y-4">
                   <div className="space-y-2 mb-2">
                     <label className="text-[11px] font-semibold text-zinc-500 block">Família da Fonte (R$)</label>
                     <FontSelect value={activeCfg.priceBadge.currencyFontFamily || 'Montserrat, sans-serif'} onChange={v => up({ priceBadge: { currencyFontFamily: v } })} googleFonts={GOOGLE_FONTS} customFonts={customFonts} />
                   </div>
                   <ColorSelector label="R$ (Moeda)" color={activeCfg.priceBadge.currencyColor} onChange={c => up({ priceBadge: { currencyColor: c } })} />
                   <div className="space-y-2">
                 ...
                 <div className="space-y-4 border-t border-zinc-800/50 pt-5">
                   <div className="space-y-2 mb-2">
                     <label className="text-[11px] font-semibold text-zinc-500 block">Família da Fonte (Valor)</label>
                     <FontSelect value={activeCfg.priceBadge.valueFontFamily || 'Montserrat, sans-serif'} onChange={v => up({ priceBadge: { valueFontFamily: v } })} googleFonts={GOOGLE_FONTS} customFonts={customFonts} />
                   </div>
          ...
          {activeCategory === 'text' && (
            <Section label="Nome do Produto" icon={PenTool} isOpen={true} onToggle={() => {}}>
                <div className="space-y-4">
                   <FontSelect value={activeCfg.descConfig.fontFamily} onChange={v => up({ descConfig: { fontFamily: v } })} googleFonts={GOOGLE_FONTS} customFonts={customFonts} />
          ...
          {activeCategory === 'suffix' && (
            <Section label="Sufixo (kg/cada)" icon={Layers} isOpen={true} onToggle={() => {}}>
              <div className="space-y-5">
                 ...
                 <div className="space-y-2 mt-4">
                   <label className="text-[11px] font-semibold text-zinc-500 block">Família da Fonte</label>
                   <FontSelect value={activeCfg.priceBadge.suffixFontFamily || 'Montserrat, sans-serif'} onChange={v => up({ priceBadge: { suffixFontFamily: v } })} googleFonts={GOOGLE_FONTS} customFonts={customFonts} />
                 </div>
          ...
          {activeCategory === 'image' && (
            <div className="p-4 bg-zinc-900/50 border border-zinc-800 rounded-xl text-zinc-400 text-[12px] text-center font-medium">
                Você selecionou a imagem do produto. Você pode redimensioná-la e reposicioná-la diretamente no palco usando as bordas de seleção vermelhas.
            </div>
          )}
        </div>
      </div>
          ...
          <svg ref={svgRef} width={config.width} height={config.height} viewBox={`0 0 ${config.width} ${config.height}`} style={{ background: 'white', userSelect: 'none' }} className="shadow-2xl ring-1 ring-zinc-800/50 rounded-lg bg-zinc-900">
            ...
            {(() => {
              ...
              return pageSlots.map((s, idx) => {
                ...
                const p = products?.[gIdx]; const name = p?.name || "NOME PRODUTO"; const price = p?.price ? p.price.replace(/R\$\s*/gi, '').trim() : "10,99";
                const v = { ...el };
                ...
                return (
                  <g key={gIdx} onClick={e => e.stopPropagation()}>
                    ...
                    <DragBox id="currency" el={v.currency} zoom={zoom} isPrimary={selectedElem === 'currency'} isSel={isSlotSel && selectedElems.includes('currency')} onStartDrag={onStartDrag} onStartResize={onStartResize} slotIdx={s.id} isDragging={!!dragging}>
                      <text x={v.currency.x + v.currency.w/2} y={v.currency.y + v.currency.h/2} fontSize={cfg.priceBadge.currencyFontSize*el.sFactor} fill={cfg.priceBadge.currencyColor} fontWeight="900" textAnchor="middle" dominantBaseline="central" pointerEvents="none" fontFamily={cfg.priceBadge.currencyFontFamily || 'Montserrat'}>R$</text>
                    </DragBox>
                    ...
                    <DragBox id="value" el={v.value} zoom={zoom} isPrimary={selectedElem === 'value'} isSel={isSlotSel && selectedElems.includes('value')} onStartDrag={onStartDrag} onStartResize={onStartResize} slotIdx={s.id} isDragging={!!dragging}>
                      <text x={v.value.x + v.value.w/2} y={v.value.y + v.value.h/2} fontSize={cfg.priceBadge.valueFontSize*el.sFactor} fill={cfg.priceBadge.valueColor} fontWeight="900" textAnchor="middle" dominantBaseline="central" pointerEvents="none" fontFamily={cfg.priceBadge.valueFontFamily || 'Montserrat'}>{price}</text>
                    </DragBox>
                    ...
                    {cfg.priceBadge.showSuffix && (
                      <DragBox id="suffix" el={v.suffix} zoom={zoom} isPrimary={selectedElem === 'suffix'} isSel={isSlotSel && selectedElems.includes('suffix')} onStartDrag={onStartDrag} onStartResize={onStartResize} slotIdx={s.id} isDragging={!!dragging}>
                        {(() => {
                          const rawSuffix = cfg.priceBadge.suffixText || 'cada';
                          const cleanSuffix = rawSuffix.toLowerCase() === 'kg' ? 'KG' : rawSuffix.toLowerCase() === 'cada' ? 'cada' : rawSuffix;
                          return <text x={v.suffix.x + v.suffix.w/2} y={v.suffix.y + v.suffix.h/2} fontSize={cfg.priceBadge.suffixFontSize*el.sFactor} fill={cfg.priceBadge.suffixColor} fontWeight="600" textAnchor="middle" dominantBaseline="central" pointerEvents="none" fontFamily={cfg.priceBadge.suffixFontFamily || cfg.priceBadge.valueFontFamily || 'Montserrat'}>{cleanSuffix}</text>;
                        })()}
                      </DragBox>
                    )}
                  </g>
            ...
            <Input
              ...
              onKeyDown={(e) => { 
                if (e.key === 'Enter' && newPresetName) {
                    const srcSlot = slots.find(s => s.id === selectedSlotId);
                    setPresets([...(presets||[]), { id: crypto.randomUUID(), name: newPresetName, priceBadge: activeCfg.priceBadge, descConfig: activeCfg.descConfig, client: selectedClientName, sourceWidth: srcSlot?.width || 500 }]);
