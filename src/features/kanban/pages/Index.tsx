...
const Index = () => {
  ...
  const visibleEmployees = useMemo(() => {
    if (loggedUserRole !== 'GUEST') return employees;
    if (!loggedUserClientLink) return [];
    
    const clientIds = loggedUserClientLink.split(',').map(id => id.trim());
    const linkedIds = new Set<string>();
    
    // Check Calendar Tasks
    if (Array.isArray(calendarTasks)) {
      calendarTasks.forEach(task => {
        if (clientIds.includes(task.calendarClientId)) {
          if (task.employeeId) linkedIds.add(task.employeeId);
          if ((task as any).employee_id) linkedIds.add((task as any).employee_id);
        }
      });
    }
    ...
    if (Array.isArray(kanbanCards)) {
      kanbanCards.forEach(card => {
        if (clientIds.includes(card.calendarClientId || '') || clientIds.includes((card as any).clientId || '')) {
          if (card.employeeId) linkedIds.add(card.employeeId);
          if (card.assignedUsers && Array.isArray(card.assignedUsers)) {
            card.assignedUsers.forEach(u => linkedIds.add(u.id));
          }
        }
      });
    }
  ...
  return (
    <>
      <div className="flex-1 min-h-0 bg-[#020202] p-4 lg:p-7 space-y-5 overflow-y-auto overflow-x-hidden w-full max-w-full custom-scrollbar dashboard-main-container">
        {/* Top Header */}
        <div className="flex flex-col md:flex-row md:items-center justify-between animate-fade-in gap-3 min-h-12 px-1 w-full max-w-full overflow-hidden">
          <div className="flex items-center gap-3 md:gap-6 flex-wrap">
            <img src={logoSrc} alt="Logo" className="h-10 md:h-14 w-auto object-contain transition-all hover:scale-110 drop-shadow-[0_0_25px_hsl(var(--primary)/0.35)] shrink-0" />
            <h1 className="text-lg md:text-2xl font-black tracking-tighter flex items-center text-white italic truncate">
              MAC V2 FINAL <span className="text-white/10 font-thin mx-2 md:mx-3 not-italic">/</span> <span className="opacity-30 text-xs md:text-base font-bold tracking-normal not-italic truncate">Gestão Operacional</span>
            </h1>
          </div>
          <div className="flex flex-wrap items-center gap-2.5 max-w-full">
            {(loggedUserRole !== 'GUEST' || (loggedUserRole === 'GUEST' && loggedUserClientLink && loggedUserClientLink.split(',').length > 1)) && (
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button className="glass-card px-3 py-1.5 md:px-4 md:py-2 flex items-center gap-2 md:gap-3 hover:bg-white/5 transition-all outline-none border-white/[0.03] shadow-2xl">
                    <Users className="w-3.5 h-3.5 text-primary shrink-0" />
                    <span className="text-[9px] font-black uppercase tracking-[0.15em] truncate">{selectedClient?.name || 'Todos Clientes'}</span>
                    <ChevronDown className="w-3 h-3 text-white/20 shrink-0" />
                  </button>
                </DropdownMenuTrigger>
                <DropdownMenuContent className="glass-card border-white/10 bg-[#0a0a0a]/98 w-64 backdrop-blur-3xl p-1.5 shadow-[0_30px_60px_rgba(0,0,0,1)] z-[999]">
            ...
            {loggedUserRole === 'GUEST' && selectedClient && (!loggedUserClientLink || loggedUserClientLink.split(',').length <= 1) && (
              <div className="glass-card px-3 py-1.5 md:px-4 md:py-2 flex items-center gap-2 border-white/[0.03] shadow-2xl opacity-60">
                <Users className="w-3.5 h-3.5 text-primary shrink-0" />
                <span className="text-[9px] font-black uppercase tracking-[0.15em] truncate">{selectedClient.name}</span>
              </div>
            )}

            <div className="flex items-center gap-2">
              <div className="glass-card px-3 py-1.5 md:px-4 md:py-2 flex items-center gap-2 md:gap-3 border-white/[0.03] shadow-2xl">
                <CalendarIcon className="w-4 h-4 text-primary shrink-0" />
                <span className="text-[9px] font-black uppercase tracking-[0.15em] whitespace-nowrap">{format(viewDate, "MMMM yyyy", { locale: ptBR })}</span>
              </div>
              {loggedUserRole !== 'GUEST' && (
                <div className="flex gap-1 shrink-0">
                  <button 
                    onClick={handlePrevMonth}
                    className="h-8 w-8 md:h-9 md:w-9 flex items-center justify-center glass-card hover:bg-white/10 hover:text-white transition-all text-white/30"
                  >
        ...
        <div className="bg-[#0A0A0B] flex flex-col border border-white/5 rounded-[1.8rem] shadow-2xl animate-fade-in group/team-section overflow-hidden mt-2">
          ...
          >
            <div className="flex flex-nowrap items-center gap-8 min-w-max pb-4 px-1">
              {visibleEmployees.slice(0, 15).map((emp, idx) => (
                <div 
                  key={emp.id} 
                  onClick={() => {
                    if (loggedUserRole !== 'GUEST') {
