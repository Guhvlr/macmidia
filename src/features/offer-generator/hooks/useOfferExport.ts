...
export function useOfferExport({
  ...
  const cleanSvgForExport = async (svgEl: SVGSVGElement): Promise<string> => {
    ...
    // Preserve original font-family values; fallbacks are now handled at the React component level
  ...
  const exportAllPages = async (format: 'svg' | 'png') => {
    ...
    try {
      ...
      if (format === 'svg') {
        ...
        const embeddedStyles = new Set<string>();
        ...
        for (let pg = 0; pg < pageCount; pg++) {
          ...
          // Robustly extract all <defs> and <style> elements from the parsed SVG
          const defsEls = Array.from(svgDoc.querySelectorAll('defs, style'));
          defsEls.forEach(el => {
            const name = el.nodeName.toLowerCase();
            if (name === 'defs') {
              Array.from(el.querySelectorAll('style')).forEach(s => {
                const text = s.textContent || '';
                if (text && !embeddedStyles.has(text)) {
                  embeddedStyles.add(text);
              ...
              if (text && !embeddedStyles.has(text)) {
                embeddedStyles.add(text);
                defsEl.appendChild(el.cloneNode(true));
              }
            }
            if (el.parentNode) el.parentNode.removeChild(el);
          });

          // Move the remaining visible elements into the group
          while (svgDoc.firstChild) {
            g.appendChild(svgDoc.firstChild);
          }
