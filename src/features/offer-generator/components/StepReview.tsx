...
import { Loader2, Trash2, Search, PlusCircle, CheckCircle2, ChevronRight, X, Sparkles, AlertTriangle, Barcode, FileText, Crop, Undo2, CheckSquare, Image as ImageIcon, ZoomIn, Zap, Layers, Plus, GripVertical, Crown } from 'lucide-react';
import { 
  DndContext, 
  closestCenter,
  KeyboardSensor,
...
export const StepReview = () => {
  ...
  const debouncedManualSearch = useDebounce(manualSearch, 500);

  React.useEffect(() => {
    if (!showVariationsFor || !debouncedManualSearch) {
      if (debouncedManualSearch?.length < 2) setManualResults([]);
      return;
    }
    const currentProduct = products.find(p => p.id === showVariationsFor);
    ...
    const fetchManualSearch = async () => {
      setIsSearchingManual(true);
      try {
        const { data: fuzzyData, error: fuzzyError } = await supabase.rpc('search_products_fuzzy', {
          search_text: debouncedManualSearch,
        ...
        const { data: ilikeData, error: ilikeError } = await supabase
          .from('products')
          .select('*')
          .or(`name.ilike.%${debouncedManualSearch}%,brand.ilike.%${debouncedManualSearch}%,category.ilike.%${debouncedManualSearch}%`)
          .limit(30);

        if (fuzzyError && ilikeError) throw ilikeError || fuzzyError;

        const combined = [...(fuzzyData || []), ...(ilikeData || [])];
        ...
        setManualResults(uniqueResults.map(v => {
          let imageUrl = '';
          if (v.image_path) {
    ...
    fetchManualSearch();
  }, [debouncedManualSearch, showVariationsFor, products]);
  ...
  const handleManualSearch = (term: string, currentProduct: ProductItem) => {
  ...
  const promoteToMain = (productId: string, imgIndex: number) => {
    setProducts(prev => prev.map(p => {
      if (p.id === productId && imgIndex > 0 && imgIndex < p.images.length) {
        const newImgs = [...p.images];
        const [promoted] = newImgs.splice(imgIndex, 1);
        newImgs.unshift(promoted);
        return { ...p, images: newImgs };
      }
      return p;
    }));
    toast.success('Imagem promovida a principal!');
  };
                        ...
                        {p.images.map((img, i) => (
                          <div key={`cur-${i}`} className={`relative p-3 rounded-2xl border transition-all flex gap-3 items-center group ${i === 0 ? 'bg-red-500/5 border-red-500/30' : 'bg-zinc-900 border-zinc-800/60'}`}>
                            ...
                            <div className="flex items-center gap-1">
                              {i > 0 && (
                                <button onClick={() => promoteToMain(p.id, i)} title="Tornar Principal" className="w-8 h-8 bg-amber-500/10 hover:bg-amber-500 text-amber-500 hover:text-white rounded-lg flex items-center justify-center transition-all opacity-0 group-hover:opacity-100">
                                  <Crown className="w-3.5 h-3.5" />
                                </button>
                              )}
                              {p.images.length > 1 && (
                                <button onClick={() => removeVariation(p.id, i)} title={i === 0 ? 'Excluir Principal' : 'Excluir Variação'} className="w-8 h-8 bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white rounded-lg flex items-center justify-center transition-all opacity-0 group-hover:opacity-100">
                        ...
                        {showVariationCreate ? (
                          ...
                          <div className="space-y-6 animate-in fade-in duration-300">
                            {manualResults.length > 0 && (
                              <div>
                                 <h4 className="text-[12px] font-semibold text-zinc-400 mb-4 flex items-center gap-2">
                                   <Search className="w-3.5 h-3.5" /> Resultados Encontrados
                                 </h4>
                                 
                                 {Object.entries(
                                   manualResults.reduce((acc, curr) => {
                                     const group = curr.brand ? curr.brand.toUpperCase() : 'OUTROS PRODUTOS';
                                     if (!acc[group]) acc[group] = [];
                                     acc[group].push(curr);
                                     return acc;
                                   }, {} as Record<string, typeof manualResults>)
                                 ).map(([groupName, items]) => (
                                   <div key={groupName} className="mb-6">
                                     <h5 className="text-[11px] font-bold text-zinc-500 mb-3 uppercase tracking-wider pl-1 border-l-2 border-red-500">{groupName}</h5>
                                        ...
                                        {items.map(v => (
                                          <div key={v.id || v.ean} className="bg-zinc-900/30 border border-zinc-800/60 rounded-2xl p-4 flex flex-col gap-3 group transition-all">
                                            ...
                                            <div className="flex-1 flex flex-col justify-between gap-2">
                                              ...
                                              {v.ean && <p className="text-[9px] text-zinc-500 text-center font-mono truncate" title={v.ean}><Barcode className="w-3 h-3 inline mr-1" />{v.ean}</p>}
                                              {v.client_name && <p className="text-[9px] text-zinc-500 text-center truncate">{v.client_name}</p>}
