    ...
    for (let i = 0; i < parsedItems.length; i += chunkSize) {
      ...
      const chunkPromises = chunk.map(async (item) => {
        ...
        if (item.mode === 'barcode') {
          ...
          // Extrair a descrição original para preservar o texto do cliente
          let exactDesc = (item.original || '').replace(/^\d+\s*/, '').trim();
          const priceMatches = [...exactDesc.matchAll(/(?:R\$\s*)?(\d+[,.]\d{2})/gi)];
          if (priceMatches.length > 0) {
            const lastPrice = priceMatches[priceMatches.length - 1][0];
            const priceIndex = exactDesc.lastIndexOf(lastPrice);
            if (priceIndex > -1) {
              exactDesc = exactDesc.substring(0, priceIndex).trim();
            }
          }
          
          const finalDisplayName = exactDesc || item.display_name || selectedMatch.name;
