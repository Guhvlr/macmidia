...
import { embedGoogleFontsInSvg } from '../utils/fonts';
...
const getElems = (slot: any, cfg: any) => {
  ...
  return {
    ...
    name: { x: slot.x + slot.width * d.offsetX / 100 - slot.width * 0.4, y: slot.y + slot.height * d.offsetY / 100 - (d.fontSize * sf * 0.7), w: slot.width * 0.8, h: d.fontSize * sf * 3.5 },
...
export const FullEditor = ({ onClose }: EditorProps) => {
  ...
  const draggingRef = useRef<ElemId | null>(null);
  ...
  const onMouseMove = (e: React.MouseEvent) => {
    ...
    if (!draggingRef.current || selectedSlotId === null) return;
    const c = toSvg(e);
    const ps = slots.find(s => s.id === selectedSlotId);
    if (!ps) return;
    const el = (getElems(ps, getSlotSettings(selectedSlotId)) as any)[draggingRef.current];
    setDragState({ dx: c.x - dragOff.x - el.x, dy: c.y - dragOff.y - el.y });
  };

  const onMouseUp = () => {
    if (isPanning) { setIsPanning(false); return; }
    if (dragState && selectedSlotId !== null && draggingRef.current) {
      const { dx, dy } = dragState;
      const ps = slots.find(s => s.id === selectedSlotId);
      if (!ps) return;
      const cfg = getSlotSettings(selectedSlotId);
      let up: any = {};
      if (draggingRef.current === 'image') up.imageConfig = { ...cfg.imageConfig, offsetX: cfg.imageConfig.offsetX + (dx / ps.width) * 100, offsetY: cfg.imageConfig.offsetY + (dy / ps.height) * 100 };
      else if (draggingRef.current === 'name') up.descConfig = { ...cfg.descConfig, offsetX: cfg.descConfig.offsetX + (dx / ps.width) * 100, offsetY: cfg.descConfig.offsetY + (dy / ps.height) * 100 };
      else if (draggingRef.current === 'badge') up.priceBadge = { ...cfg.priceBadge, badgeOffsetX: cfg.priceBadge.badgeOffsetX + (dx / ps.width) * 100, badgeOffsetY: cfg.priceBadge.badgeOffsetY + (dy / ps.height) * 100 };
        ...
        <div style={{ transform: `translate(${panOffset.x}px, ${panOffset.y}px) scale(${zoom})`, transformOrigin: 'center center' }}>
          ...
          >
            ...
            {(() => {
              ...
              return slots.map((slot, originalIdx) => {
                if ((slot.pageIndex || 0) !== activePage) return null;
                ...
                const v = { ...el };
                if (isSelected && dragState && draggingRef.current) {
                  const dType = draggingRef.current;
                  if (dType === 'badge') {
                    v.badge = { ...v.badge, x: v.badge.x + dragState.dx, y: v.badge.y + dragState.dy };
                  } else {
                    (v as any)[dType] = { ...(v as any)[dType], x: (v as any)[dType].x + dragState.dx, y: (v as any)[dType].y + dragState.dy };
                  }
                }
                ...
                return (
                  ...
                  <g style={{ cursor: 'move' }} onMouseDown={e => onStartDrag(e, 'badge', slot.id)}>
                    ...
                    <text x={v.badge.x + (v.badge.w * pb.valueOffsetX / 100)} y={v.badge.y + (v.badge.h * pb.valueOffsetY / 100)} dy=".35em" fontSize={pb.valueFontSize * sf} fill={pb.valueColor} fontWeight="900" textAnchor="middle" fontFamily={pb.valueFontFamily} pointerEvents="none">{product.price.replace(/R\$\s*/gi, '').trim()}</text>
                    {pb.showSuffix && (() => {
                      ...
                      return <text x={v.badge.x + (v.badge.w * pb.suffixOffsetX / 100)} y={v.badge.y + (v.badge.h * pb.suffixOffsetY / 100)} dy=".35em" fontSize={pb.suffixFontSize * sf} fontFamily={pb.suffixFontFamily || pb.valueFontFamily || 'Montserrat'} fill={pb.suffixColor} fontWeight="600" textAnchor="middle" pointerEvents="none">{cleanSuffix}</text>;
                    })()}
...
export const StepFinal = () => {
  const { config, slots, products, pageCount, customFonts, getSlotSettings, customCanvasElements, slotSettings, activeProjectId, activeProjectName } = useOffer();
  const navigate = useNavigate();
  const svgRefs = useRef<(SVGSVGElement | null)[]>([]);
  const [isProcessing, setIsProcessing] = useState(false);
  const [exportFilename, setExportFilename] = useState('tabloide_macmidia');

  // ── Cross-tab sync: receive editor changes via BroadcastChannel only ──
  useEffect(() => {
    let channel: BroadcastChannel | null = null;
    try {
      channel = new BroadcastChannel('macmidia_offer_editor');
      channel.onmessage = (event) => {
        if (event.data?.type === 'EDITOR_SAVED' && event.data.snapshot) {
          // Remove step and activePage so the user stays on Step 5
          const { step: _s, activePage: _ap, ...safeSnapshot } = event.data.snapshot;
          useOfferStore.getState().loadProjectState(safeSnapshot);
          // Ensure step stays at 5 after loadProjectState
          useOfferStore.setState({ step: 5 });
          toast.success('Alterações do editor sincronizadas!');
        }
      };
    } catch (e) {
    ...
    // Clean up any stale sync data
    localStorage.removeItem('macmidia_offer_sync');
  ...
  const processSvgForExport = async (svg: SVGSVGElement, format: 'svg' | 'png') => {
    ...
    // Embed Google Fonts as base64 into SVG defs
    const customFontNames = customFonts.map(f => f.name);
    await embedGoogleFontsInSvg(clone, customFontNames);
  ...
  const getUniversalStack = (customEls: any[], pageIdx: number) => {
    const stack: any[] = [];
    ...
    const slotsBeforePage = slots.filter(s => (s.pageIndex || 0) < pageIdx).length;
    
    // Get only the slots for this page, preserving their original array index for slot ID lookups
    let localSlotIndex = 0;
    slots.forEach((slot, originalIdx) => {
      if ((slot.pageIndex || 0) !== pageIdx) return;
  ...
  const handleOpenEditor = () => {
    try {
      ...
      const editorData = { ...snapshot, activeProjectId, activeProjectName };
            ...
            <div key={i} className="flex flex-col items-center">
              ...
              <div className="relative shadow-xl rounded-2xl overflow-hidden border border-zinc-800 transition-all bg-zinc-900/20">
                <svg ref={el => svgRefs.current[i] = el} width="100%" viewBox={`0 0 ${config.width} ${config.height}`} className="w-full h-auto block bg-white">
                  ...
                  {getUniversalStack(customCanvasElements[i] || [], i).map(item => {
                    if (item.isTemplate) {
                      ...
                      return (
                        <g key={item.id}>
                          {type === 'image' && product.images?.[0] && <image href={product.images[0]} x={elData.image.x} y={elData.image.y} width={elData.image.w} height={elData.image.h} preserveAspectRatio="xMidYMid meet" />}
                          {type === 'image1' && product.images?.[1] && <image href={product.images[1]} x={elData.image1?.x || elData.image.x} y={elData.image1?.y || elData.image.y} width={elData.image1?.w || elData.image.w} height={elData.image1?.h || elData.image.h} preserveAspectRatio="xMidYMid meet" />}
                          {type === 'image2' && product.images?.[2] && <image href={product.images[2]} x={elData.image2?.x || elData.image.x} y={elData.image2?.y || elData.image.y} width={elData.image2?.w || elData.image.w} height={elData.image2?.h || elData.image.h} preserveAspectRatio="xMidYMid meet" />}
                          {type === 'badgeBg' && (pb.badgeImageUrl ? <image href={pb.badgeImageUrl} x={elData.badgeBg.x} y={elData.badgeBg.y} width={elData.badgeBg.w} height={elData.badgeBg.h} preserveAspectRatio="xMidYMid meet" /> : <rect x={elData.badgeBg.x} y={elData.badgeBg.y} width={elData.badgeBg.w} height={elData.badgeBg.h} rx={pb.borderRadius * sf} fill={pb.bgColor} />)}
                          {type === 'badgeCurrency' && <text x={elData.badgeCurrency.x + elData.badgeCurrency.w/2} y={elData.badgeCurrency.y + elData.badgeCurrency.h/2} fontSize={pb.currencyFontSize * sf} fill={pb.currencyColor} fontWeight="900" fontFamily={pb.currencyFontFamily} textAnchor="middle" dominantBaseline="central">R$</text>}
                          {type === 'badgeValue' && <text x={elData.badgeValue.x + elData.badgeValue.w/2} y={elData.badgeValue.y + elData.badgeValue.h/2 + (pb.valueFontSize * sf * 0.3)} fontSize={pb.valueFontSize * sf} fill={pb.valueColor} fontWeight="900" textAnchor="middle" fontFamily={pb.valueFontFamily} letterSpacing="-0.05em">{product.price.replace(/R\$\s*/gi, '').trim()}</text>}
                          {type === 'badgeSuffix' && pb.showSuffix && (() => {
                            const rawSuffix = product.suffix || pb.suffixText || 'cada';
                            const cleanSuffix = rawSuffix.toUpperCase();
                            return <text x={elData.badgeSuffix.x + elData.badgeSuffix.w/2} y={elData.badgeSuffix.y + elData.badgeSuffix.h/2 + (pb.suffixFontSize * sf * 0.3)} fontSize={pb.suffixFontSize * sf} fontFamily={pb.suffixFontFamily || pb.valueFontFamily || 'Montserrat'} fill={pb.suffixColor} fontWeight="600" textAnchor="middle">{cleanSuffix}</text>;
                          })()}
                          {type === 'name' && (
                            <text textAnchor="middle" fill={dc.color} fontSize={dc.fontSize * sf} fontWeight="800" fontFamily={dc.fontFamily || 'Montserrat'} letterSpacing="-0.02em">
                              {nameLines.map((line, lineIdx) => <tspan key={lineIdx} x={elData.name.x + elData.name.w / 2} y={nameY + lineIdx * nameLH + (dc.fontSize * sf * 0.3)}>{line}</tspan>)}
                            </text>
                          )}
                        </g>
                      );
                      ...
                      if (el.type === 'product_clone' && el.productData?.imageUrl) {
                        shape = <image href={el.productData.imageUrl} x={0} y={0} width={el.w} height={el.h} preserveAspectRatio="xMidYMid meet" />;
                      } else if (el.type === 'rect') {
                        shape = <rect x={0} y={0} width={el.w} height={el.h} fill={s.bgColor} stroke={s.borderColor} strokeWidth={s.borderWidth} rx={s.borderRadius} />;
                      } else if (el.type === 'circle') {
                        shape = <ellipse cx={el.w/2} cy={el.h/2} rx={el.w/2} ry={el.h/2} fill={s.bgColor} />;
                      } else if (el.type === 'divider') {
                        shape = <rect x={0} y={0} width={el.w} height={el.h} fill={s.bgColor} rx={4} />;
                        ...
                        shape = (
                          <g>
                            ...
                            <text x={tX} y={el.h/2} fontFamily={s.fontFamily} fontSize={s.fontSize} fontWeight={s.fontWeight} fill={s.color} textAnchor={textAnchor} dominantBaseline="middle" letterSpacing={s.letterSpacing ? s.letterSpacing + 'px' : 'normal'}>{s.uppercase ? el.data.text.toUpperCase() : el.data.text}</text>
                          </g>
                        );
                      }
                      return (
